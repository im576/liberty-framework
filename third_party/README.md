...
| [Violent Liberty](https://www.nexusmods.com/gta4/mods/1420) | Optional local blood visual companion | Inspected local archive contains no source or license | User-owned ASI/INI/WTD installed into their game only; no code or asset committed or redistributed. SHA-256 and compatibility notes: [ViolentLiberty.md](../docs/research/ViolentLiberty.md). |
...
| [DXVK GPLAsync 2.6.2 and Shader Cache](https://www.nexusmods.com/gta4/mods/385) | Optional async-shader DXVK build for the owner's GPU (no graphics pipeline library) | DXVK (zlib license) by the DXVK team, GPLAsync patches by Ph42oN, shader caches by the community and ValentynL, published by ValentynL | Owner-downloaded archives installed into their game only by `tools/install-dxvk-gplasync.ps1` (hash-pinned, backed up, reversible); nothing committed or redistributed. |
| [Realistic Weapon Overhaul Mod](https://www.nexusmods.com/gta4/mods/272) | Replacement arsenal for every vanilla firearm (W-1): models, animations, sounds, WeaponInfo stats | konarider26 (pack, WeaponInfo); models/textures lenol03 and Push; animations BDawg (Improved Animations Pack); sounds lenol03 and SparktimusPrime (Spark's Weapon Sound Revamp) | Owner-downloaded archive installed into their game only by `tools/install-weapon-pack.ps1` (hash-pinned, backed up, reversible; sounds via FusionFix RPF overload); nothing committed or redistributed. |
| [Blender](https://www.blender.org) 4.2+ (tested 5.2.2 LTS) and its bundled glTF exporter | Modelling tool; the Liberty Exporter add-on drives its glTF export | GPL-3.0-or-later | External tool, not bundled. The add-on (	ools/blender/liberty_exporter, our own code) is GPL-3.0-or-later as Blender requires for bpy add-ons; the compiler talks to it only through files |
