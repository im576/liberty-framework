...
`./tools/prepare-t007.ps1 -GameDirectory '<GTAIV folder>'` generates an ignored, local-only `staging/t007/WeaponInfo.xml` by cloning the installed vanilla pistol entry as `LF_GOLD_PISTOL`. The model and stats remain vanilla for the identity test. The live test selected custom ID 58; the vanilla pistol disappeared from the handgun inventory. The DevTools menu exposes Weapon Status, Give Test Pistol, and Give Vanilla Pistol. The owner verified the menu switch and total-ammo transfer. The XML is already installed on the tester's machine; do not rerun `deploy-t007.ps1` against the existing override. The installer refuses to overwrite an existing weapon override or install while the game runs. `remove-t007.ps1` removes only a hash-matching T-007 override while the game is closed. See [T-007](../docs/tasks/T-007-weapon-slots.md) for findings.
