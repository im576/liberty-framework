...
- 2026-09-24 — Phase 2 stream C staged on `codex/phase2-systems`: T-025 stable physical weapon IDs and metadata survive body/trunk/safehouse/owned-car state; catalog covers a vanilla non-gold sidearm pair and existing carbine add-on. A paid safehouse gunsmith choice switches a physical pistol between verified factory/gold models. T-023 body variants remain blocked pending a model/extra-slot playtest; LVS generic Extras still handles preview/purchase/owned-car restore. Offline build/checks pass; no game installation.
