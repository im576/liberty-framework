...
Status: **SUPERSEDED by [T-010](T-010-phase1-gunplay.md)** — the aim camera is driven through validated `CCamAimWeapon` fields (ADR-0004). Original card kept for history: Scope: find the smallest supported way to apply a visible, reversible pitch/yaw impulse to the player's current aim camera on one test weapon. Start with ScriptHookDotNet wrappers, then documented natives; memory requires a separate ADR and pattern scan.
