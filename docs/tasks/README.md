...
**Current assignment:** T-000, T-001, and T-002 passed owner gameplay checks. T-003's first controller menu worked but its D-pad opened the phone. T-007's first custom pistol selected as ID 58 and replaced the vanilla pistol in the handgun slot. A revised menu with weapon actions and player-control suppression is built locally for the next shutdown/playtest. The project owner performs game playtests and alone marks a tested card `DONE`.
...
| T-003 | [DevTools minimum menu](T-003-devtools-menu.md) | T-002 | NEEDS-PLAYTEST (phone conflict fix built offline; deploy after game closes) |
| T-007 | [Separate gold weapon slot spike](T-007-weapon-slots.md) | T-001 | NEEDS-PLAYTEST (ID 58 confirmed; behavior and menu switching pending) |
