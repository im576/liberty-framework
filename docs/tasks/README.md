...
**Current assignment:** T-000, T-001, and T-002 passed owner gameplay checks. T-003's minimum DevTools menu and T-007's first custom weapon identity probe are built offline and await one guarded installation/playtest after GTA IV closes. No gold weapon behavior has been implemented. The project owner performs game playtests and alone marks a tested card `DONE`.
...
| T-003 | [DevTools minimum menu](T-003-devtools-menu.md) | T-002 | NEEDS-PLAYTEST (built offline; deploy after game closes) |
| T-007 | [Separate gold weapon slot spike](T-007-weapon-slots.md) | T-001 | NEEDS-PLAYTEST (first custom pistol staged; deploy after game closes) |
